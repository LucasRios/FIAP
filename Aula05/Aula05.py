...
# Camada de Processamento — cacheada para evitar recomputação a cada rerun
@st.cache_resource
def simulate_model(model_type: str, threshold: float, seed: int): 
...
# Camada de Dados — cacheada pois o resultado depende apenas do dicionário de entrada
...
# Invalidação manual do cache — útil para reset administrativo e debug
...
st.sidebar.info(
    ...
    "human-in-the-loop e visualização de dados."
)
...
if st.session_state.analysis_ran:
    ...
    with tab_dados:
        ...
        # .copy() garante que o objeto cacheado não seja mutado acidentalmente
