...
def etapa_1_coleta(urls: list) -> pd.DataFrame:
    """
...
def etapa_2_preparacao(df: pd.DataFrame) -> pd.DataFrame:
    """
...
def etapa_analise_local(df: pd.DataFrame) -> dict:
    """
...
# =============================================================================
# Geração do Gráfico de Distribuição de Sentimentos
# =============================================================================

def gerar_grafico_sentimento(distribuicao: dict) -> plt.Figure:
    """
    Gera o gráfico de pizza com a distribuição de sentimentos.

    Retorna a figura em vez de chamar plt.show() diretamente, para que o
    Streamlit possa renderizá-la com st.pyplot(fig).

    Args:
        distribuicao (dict): {"positive": float, "neutral": float, "negative": float}
    ...
    ax.set_title("Distribuição de Sentimentos (Análise de Léxico Local)")

    # ALTERADO: era plt.show() — exibe no terminal standalone mas trava no Streamlit.
    # Retornamos a figura para que cada contexto decida como exibi-la:
    #   • Streamlit:  st.pyplot(fig)
    #   • Terminal:   fig.show() ou plt.show()
    return fig
...
def imprimir_relatorio(res: dict):
    """
    Exibe o relatório NLP formatado no terminal.

    ALTERADO: era display(Markdown(...)) — IPython.display não existe no VS Code.
    Substituído por print() com formatação em texto simples.
    """
    print("\n" + "=" * 60)
    print("📊 Relatório NLP Local (Independente de API)")
    print("=" * 60)
    print(f"\n🎯 Sentimento Geral: {res['overall_sentiment']}  (Polaridade: {res['polarity_val']:.2f})")
    print(f"\n📝 Contexto Identificado (Sumarização):\n{res['summary']}")
    print(f"\n🏷️  Temas em Alta (Palavras-Chave):\n{', '.join(res['themes'])}")
    print(f"\n📈 Distribuição:")
    for label, valor in res['distribution'].items():
        print(f"   {label.capitalize()}: {valor:.1f}%")
    print("=" * 60)
...
def run_pipeline(urls: list = None) -> tuple:
    ...
    print("=" * 60)
    df_bruto = etapa_1_coleta(urls)

    # ── Etapa 2: Preparação ───────────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("🧹 ETAPA 2 — Limpeza e Preparação")
    print("=" * 60)
    df_final_global = etapa_2_preparacao(df_bruto)
    ...
    if not df_final_global.empty:
        ...
        print("=" * 60)
        analise_global = etapa_analise_local(df_final_global)

        imprimir_relatorio(analise_global)

        # Exibe o gráfico no terminal (standalone)
        fig = gerar_grafico_sentimento(analise_global['distribution'])
        plt.show()  # No Streamlit esse trecho não é chamado — a UI usa gerar_grafico_sentimento() diretamente
    else:
