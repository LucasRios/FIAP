...
if st.session_state.analysis_ran:
    ...
    with tab_geracao:
        ...
        col1, col2 = st.columns([2, 1])  # coluna 1 ocupa 2/3, coluna 2 ocupa 1/3
        with col1:
            ...
            if st.button("🔄 Gerar Novas Imagens", type="primary"):
                with st.spinner("Gerando imagens..."):
                    # Salva imagens geradas no estado para não perder ao rerenderizar
        ...
        # Exibe imagens geradas se existirem no estado da sessão
        if st.session_state.generated_images is not None:
            st.subheader("Imagens Geradas")
            n = st.session_state.num_generated
            # Cria N colunas dinamicamente — uma por imagem gerada
            cols = st.columns(n)
            for i, col in enumerate(cols):
                with col:
                    st.image(
                        st.session_state.generated_images[i].squeeze(),  # (28,28,1)→(28,28)
                        clamp=True,
    ...
    with tab_dados:
        ...
        if not st.session_state.history_df.empty:
            ...
            # Gráfico de barras: uma barra por execução, colorida pela classificação
            fig_hist = px.bar(
                st.session_state.history_df,
                x="Execução",
                y="Erro MSE",
            ...
            # Linhas horizontais indicando os thresholds configurados
            fig_hist.add_hline(
    ...
    with tab_monitor:
        ...
        # Altair: distribuição de classificações com gramática declarativa
        # alt.Chart: define a fonte de dados
        # .mark_bar(): tipo de marca visual (barra)
        # .encode(): mapeia colunas do DataFrame para canais visuais (x, y, color)
        if len(st.session_state.history) >= 2:
            ...
            # Agrupa por classificação e conta ocorrências
            ...
            chart_classes = (
                ...
                .mark_bar()
                .encode(
                    x=alt.X("classification:N", title="Classificação"),   # :N = nominal (categórico)
                    y=alt.Y("Contagem:Q", title="Número de triagens"),    # :Q = quantitativo
                    color=alt.Color(
                        "classification:N",
                        scale=alt.Scale(
                        ...
                        legend=None,  # oculta legenda pois as cores já são autoexplicativas
                    ),
                    tooltip=["classification", "Contagem"],  # tooltip interativo ao hover
                )
                .properties(height=260)
            )
            st.altair_chart(chart_classes, use_container_width=True)

        # Plotly scatter: MSE por execução, colorido por classificação
        if len(st.session_state.history) >= 3:
            ...
            fig_scatter = px.scatter(
                ...
                hover_data=["Confiança (%)"],  # dados extras exibidos no hover
                title="Histórico de triagens",
                height=320,
            )
            # Linhas de threshold sobrepostas ao scatter para referência visual
